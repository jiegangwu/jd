				...
				if (safeGet(data)) {
					...
					if (data.data.bizCode === 0) {
						if (data.data.result.produceScore)
							console.log(
								`任务完成成功，获得：${
									data.data.result.produceScore ? data.data.result.produceScore: "未知"
								}能量`
							);
						else
							console.log(
								`任务领取结果：${data.data.bizMsg ? JSON.stringify(data):"未知"}`
							);
					} else {
						console.log(`任务完成失败：${data.data.bizMsg ? JSON.stringify(data):"未知"}`);
					}
				}
			} catch (e) {
				console.log(e);
