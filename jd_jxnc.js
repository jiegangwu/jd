...
!(async () => {
    await requireConfig();
    if (process.env.JXNC_FORBID_ACCOUNT) process.env.DREAMFACTORY_FORBID_ACCOUNT.split('&').map((item, index) => Number(item) === 0 ? cookiesArr = [] : cookiesArr.splice(Number(item) - 1 - index, 1))
    if (!cookieArr[0]) {
